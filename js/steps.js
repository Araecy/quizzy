;( function( window ) {
	...
	stepsForm.prototype._initEvents = function() {
		...
		// Laat de check knop zien als de input field is ge focused
		firstElInput.addEventListener( 'focus', onFocusStartFn );

		// laat de next question zien
		this.ctrlNext.addEventListener( 'click', function( ev ) {
			ev.preventDefault();
			self._nextQuestion();
		} );

		// Enter laat zien of het goed of fout is
	...
	stepsForm.prototype._nextQuestion = function() {
		...
		if ( this.supportsHTML5Forms ) {
			let input = this.questions[ this.current ].querySelector( 'input, textarea, select' );
			// clear error messages
			input.setCustomValidity( '' );

			if ( !input.checkValidity() ) {
				input.setCustomValidity( 'Whoops, dat is geen email address!' );
		...
		if( !this.isFilled ) {
			...
			// current question
			let nextQuestion = this.questions[ this.current ];
	...
	stepsForm.prototype._progress = function() {
		let currentProgress = this.current * ( 100 / this.questionsCount );
	...
	stepsForm.prototype._validate = function() {
		...
		let input = this.questions[ this.current ].querySelector( 'input, textarea, select' ).value;
		if( input === '' ) {
	...
	stepsForm.prototype._showError = function( err ) {
		...
		switch( err ) {
			case 'EMPTYSTR' :
				message = 'Vul iets in voordat je checkt of het goed is.';
				break;
			case 'INVALIDEMAIL' :
